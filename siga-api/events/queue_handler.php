...
function add_to_queue(int $value, string $queue_name, ?array $args): bool{
    return (bool) wp_insert_post(["post_title" => $value, "post_type" => $queue_name, "post_content" => serialize($args)]);
}

function remove_from_queue(int $value, string $queue_name): bool {
    global $wpdb;
...
function update_in_queue(int $value, string $queue_name, $value_to_update): bool{
    global $wpdb;
...
function get_queue(string $queue_name, ?int $limit = 1){
    global $wpdb;

    $sql = "SELECT post_title, post_content FROM wp_posts WHERE post_type = '%s' LIMIT %d";
    $query = $wpdb->prepare($sql, $queue_name, $limit);
